...
const nailData: Record<string, any> = {
  "1": { title: "Gel Manicure", price: 6000, img: "gel-manicure.jpg", vendor: "Queen Nails", desc: "Long-lasting gel polish with flawless shine. Up to 3 weeks.", duration: "60 mins", rating: 4.9, reviews: 387 },
  "2": { title: "Acrylic Full Set", price: 10000, img: "acrylic-nails.jpg", vendor: "Nail Bae Pro", desc: "Strong, customizable acrylics. Any length, any shape.", duration: "90 mins", rating: 4.8, reviews: 298 },
  "3": { title: "Chrome Nails", price: 8500, img: "chrome-nails.jpg", vendor: "Glow Studio", desc: "Mirror-effect chrome finish. Trending everywhere.", duration: "75 mins", rating: 4.9, reviews: 412 },
  "4": { title: "French Tips Classic", price: 5500, img: "french-tips.jpg", vendor: "Queen Nails", desc: "Timeless white tips. Clean and elegant.", duration: "45 mins", rating: 4.7, reviews: 256 },
};

export default function NailDetail() {
  const router = useRouter();
  const { id } = useParams();
  const nailId = Array.isArray(id) ? id[0] : id;
  const nail = nailData[nailId] || nailData["1"];
  const numericId = parseInt(nailId);
  ...
  const handleAddToCart = useCallback(() => {
    addToCart({ id: numericId, title: nail.title, price: nail.price, img: nail.img, category: "Nails" });
    showToast("Added to Cart!");
  }, [addToCart, numericId, nail, showToast]);
  ...
  const handleWishlist = useCallback(() => {
    if (isInWishlist) {
      ...
      addToWishlist({ id: numericId, title: nail.title, price: nail.price, img: nail.img });
      setLocalWishlist(prev => new Set(prev).add(numericId));
      showToast("Added to Wishlist", true);
    }
  }, [isInWishlist, removeFromWishlist, addToWishlist, numericId, nail, showToast]);
    ...
    <>
      ...
      <div className="p-6 space-y-8 pb-32">
        ...
        <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="relative h-80 rounded-3xl overflow-hidden shadow-2xl">
          <Image src={`/images/${nail.img}`} alt={nail.title} fill className="object-cover" />
        ...
        <div className="space-y-5">
          <div>
            <h1 className="text-3xl font-black text-gray-900">{nail.title}</h1>
            <p className="text-lg text-gray-600 mt-1">by <span className="font-bold text-purple-600">{nail.vendor}</span></p>
          </div>
          <div className="flex items-center gap-3">
            <Star className="w-6 h-6 text-yellow-500 fill-current" />
            <span className="text-xl font-black">{nail.rating}</span>
            <span className="text-gray-500">({nail.reviews} reviews)</span>
          </div>
          <p className="text-base text-gray-700 leading-relaxed">{nail.desc}</p>
          <p className="text-sm text-teal-600 font-bold">Duration: {nail.duration}</p>
          <div className="text-4xl font-black bg-gradient-to-r from-purple-600 to-teal-600 bg-clip-text text-transparent">
            ₦{nail.price.toLocaleString()}
          </div>
        </div>
